...
export function UserCreatePage() {
	...
	const saveProgress = useUserCreateStore((state) => state.saveProgress);
	const clearProgress = useUserCreateStore((state) => state.clearProgress);
	const hasSavedProgress = useUserCreateStore(
		(state) => state.hasSavedProgress,
	);
	const loadProgress = useUserCreateStore((state) => state.loadProgress);
	...
	useEffect(() => {
		...
		if (hasSaved) {
			const timestamp = FormPersistence.getSavedTimestamp("USER");
		...
		>
			<div className="space-y-6">
				...
				<p className="text-muted-foreground text-xs">
					Need to pause? Click "Save Progress" to save your work. If you don't
					save, you'll lose your progress when you navigate away or refresh the
					page.
				</p>
			</div>
		</UserPageTitle>
