...
import type {
	OrganizationColumn,
	OrganizationColumnKey,
	OrganizationTableRow,
} from "./columns";

export type SortState = {
	field: OrganizationColumnKey;
	direction: "asc" | "desc";
};
...
const SelectionCell = ({
	...
	return (
		<TableCell className="w-[40px] px-3 py-2">
			<Checkbox
				aria-label={`Select organization ${rowId}`}
				checked={checked}
				onCheckedChange={handleChange}
			/>
		</TableCell>
...
const ActionsCell = ({
	...
	const isActive = row.status === ORGANIZATION_STATUS.ACTIVE;
	const isPending = row.status === ORGANIZATION_STATUS.PENDING;
	const isSuspended = row.status === ORGANIZATION_STATUS.SUSPENDED;
	...
	return (
		<>
			<TableCell className="w-[48px] text-right px-3 py-2">
				<DropdownMenu>
					...
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							{(() => {
								switch (pendingAction?.type) {
									case "approve":
										return `Are you sure you want to approve "${row.name}"? This will activate the organization and set both status and KYC to approved.`;
									case "reject":
										return `Are you sure you want to reject "${row.name}"? This will set both status to rejected and KYC to rejected.`;
									case "suspend":
										return `Are you sure you want to suspend "${row.name}"? This will suspend the organization and all its users.`;
									case "delete":
...
export const DataTable = ({
	...
	const handleSortClick = (key: OrganizationColumnKey, sortable?: boolean) => {
		if (!sortable) {
			return;
		}
	...
	const renderHeaderContent = (column: OrganizationColumn): ReactNode => {
		if (!column.sortable) {
		...
		<div className="relative">
			<div className="w-full">
				<Table>
					...
					<TableCaption>
						{isFetching ? "Refreshing organizations…" : ""}
					</TableCaption>
				</Table>
			</div>
			{isLoading ? (
				<div className="absolute inset-0 flex items-center justify-center bg-background/60">
					<Badge variant="secondary">Loading organizations…</Badge>
				</div>
			) : null}
		</div>
	);
