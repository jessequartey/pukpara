...
type DataTableProps = {
	...
	onEdit?: (row: FarmerTableRow) => void;
	onViewFarms?: (row: FarmerTableRow) => void;
	isLoading?: boolean;
	isFetching?: boolean;
};
		...
		<TableCell className="w-[56px] text-right px-3 py-2">
			<DropdownMenu>
				...
				<DropdownMenuContent align="end">
					...
					{onViewFarms && (
						<DropdownMenuItem onSelect={() => onViewFarms(row)}>
							View farms ({row.farmCount})
						</DropdownMenuItem>
					)}
					{onEdit && (
						<>
...
export const DataTable = ({
	...
	const handleSortClick = (key: FarmerColumnKey, sortable?: boolean) => {
	...
	const renderHeaderContent = (column: FarmerColumn): ReactNode => {
		if (!column.sortable) {
		...
		<div className="relative">
			<div className="overflow-x-auto">
				<Table>
					...
					<TableCaption>{isFetching ? "Refreshing farmers…" : ""}</TableCaption>
				</Table>
			</div>
			{isLoading && (
