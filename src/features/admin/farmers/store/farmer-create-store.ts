...
export const useFarmerCreateStore = create<FarmerCreateStoreState>(
	(set, get) => ({
			...
			set((state) => {
				...
				const updatedFarmers = state.bulkUpload.parsedFarmers.map((farmer) => {
					...
					return {
						...
						errors: farmerErrors,
						isValid:
							farmerErrors.length === 0 &&
							validatedFarms.every((f) => f.isValid),
		...
		saveProgress: () => {
			...
			FormPersistence.save("FARMER", withTimestamp(dataToSave));
		},
		loadProgress: () => {
			const savedData = FormPersistence.load("FARMER");
			...
			set({
				step: data.step || 1,
				mode: data.mode || null,
				farmer: { ...initialFarmerState, ...data.farmer },
				farms: data.farms || [],
				bulkUpload: { ...initialBulkUploadState, ...data.bulkUpload },
				selectedOrganization: data.selectedOrganization || null,
			});
			return true;
		},
		clearProgress: () => {
			FormPersistence.clear("FARMER");
		},
		hasSavedProgress: () => {
			return FormPersistence.hasSavedData("FARMER");
		},
		reset: () =>
			set({
				step: 1,
