...
const farmHeaders = [
  'farmName', 'acreage', 'cropType', 'soilType', 'locationLat', 'locationLng'
];
...
const sampleFarms = [
  // Farmer 1 farms
  { farmName: 'Main Cocoa Farm', acreage: 5.5, cropType: 'Cocoa', soilType: 'loamy', locationLat: 6.0769, locationLng: -0.8761 },
  { farmName: 'Rice Paddies', acreage: 2.3, cropType: 'Rice', soilType: 'clay', locationLat: 6.0785, locationLng: -0.8745 },
  { farmName: 'Vegetable Garden', acreage: 1.2, cropType: 'Mixed Vegetables', soilType: 'sandy', locationLat: 6.0801, locationLng: -0.8729 },

  // Farmer 2 farms
  { farmName: 'Cassava Plantation', acreage: 8.7, cropType: 'Cassava', soilType: 'loamy', locationLat: 6.1234, locationLng: -0.7890 },
  { farmName: 'Maize Field', acreage: 4.1, cropType: 'Maize', soilType: 'silt', locationLat: 6.1250, locationLng: -0.7874 },
  { farmName: 'Plantain Grove', acreage: 3.6, cropType: 'Plantain', soilType: 'clay', locationLat: 6.1266, locationLng: -0.7858 },

  // Farmer 3 farms
  { farmName: 'Oil Palm Estate', acreage: 12.0, cropType: 'Oil Palm', soilType: 'loamy', locationLat: 6.2100, locationLng: -0.6500 },
  { farmName: 'Pepper Farm', acreage: 0.8, cropType: 'Pepper', soilType: 'sandy', locationLat: 6.2116, locationLng: -0.6484 },
  { farmName: 'Yam Farm', acreage: 6.2, cropType: 'Yam', soilType: 'rocky', locationLat: 6.2132, locationLng: -0.6468 }
];
...
const farmersData = [
  ...
  farmerHeaders.map(header => {
    const requiredFields = ['firstName', 'lastName', 'phone', 'dateOfBirth', 'gender',
                           'community', 'address', 'districtName', 'organizationName',
                           'idType', 'idNumber'];
    return requiredFields.includes(header) ? `${header}*` : header;
  }),
  // Sample data rows
  ...sampleFarmers.map(farmer => farmerHeaders.map(header => farmer[header] || ''))
];
...
const farmsData = [
  // Headers row
  ['farmerRow*', ...farmHeaders.map(header => header === 'farmName' ? `${header}*` : header)],
